...
export default function TicketListPage() {
  ...
  const getSlaBgColor = (color) => {
    switch (color) { case 'red': return 'bg-red-50 text-red-700 border-red-200'; case 'orange': return 'bg-orange-50 text-orange-700 border-orange-200'; case 'yellow': return 'bg-yellow-50 text-yellow-700 border-yellow-200'; default: return 'bg-green-50 text-green-700 border-green-200'; }
  };
              ...
              tickets.map(t => (
                <div key={t.id} onClick={() => selectTicket(t)} className={`bg-white rounded-xl shadow-sm border p-4 cursor-pointer hover:shadow-md transition ${selectedTicket?.id === t.id ? 'ring-2 ring-teal-500 border-teal-500' : 'border-gray-100'}`}>
                  <div className="flex justify-between items-start mb-2">
                    <div className="flex items-center gap-2 flex-1 min-w-0"><span className="text-xs">{priorityIcons[t.priority]}</span><h3 className="font-bold text-sm truncate">{t.subject}</h3></div>
            ...
            {selectedTicket ? (
              <div className="bg-white rounded-xl shadow-sm border border-gray-100 flex flex-col h-full">
                <div className="p-4 border-b flex-shrink-0">
                  <div className="flex justify-between items-start mb-2"><h2 className="font-bold text-sm">{selectedTicket.subject}</h2>{typing && <span className="text-xs text-gray-400 animate-pulse">Client écrit...</span>}</div>
                  {selectedTicket.sla && (
                    <div className={`flex items-center gap-2 p-2 rounded-lg border text-xs ${getSlaBgColor(selectedTicket.sla.sla_color)}`}>
                      ...
                      <div className="flex-1">
                        <div className="flex justify-between"><span className="font-semibold">{selectedTicket.sla.priority_label} · {selectedTicket.sla.sla_message}</span><span className="font-bold">{selectedTicket.sla.elapsed_minutes} min</span></div>
                        <div className="w-full h-1 bg-white/50 rounded-full mt-1"><div className={`h-full rounded-full ${getSlaColor(selectedTicket.sla.sla_color)}`} style={{ width: `${Math.min(100, selectedTicket.sla.remaining_percent)}%` }} /></div>
                      </div>
                    </div>
                  )}
                </div>
                        ...
                        <div className={`max-w-[85%] p-2.5 rounded-2xl ${style.bg} relative`}>
                          ...
                          {!isBot && !isEditing && (
                            <div className="hidden group-hover:flex absolute -top-2 right-2 gap-1">
                              {isMine && <button onClick={(e) => { e.stopPropagation(); startEdit(m); }} className="bg-blue-500 text-white rounded-full w-5 h-5 flex items-center justify-center text-[10px]">✎</button>}
                              <button onClick={(e) => { e.stopPropagation(); deleteMessage(m.id); }} className="bg-red-500 text-white rounded-full w-5 h-5 flex items-center justify-center text-[10px]">✕</button>
                            </div>
                          )}
                        </div>
                      </div>
                ...
                <>
                  ...
                  <div className="flex-1 overflow-y-auto p-3 space-y-3 bg-gray-50">
                    {messages.map((m, i) => {
                      ...
                      const style = getMessageStyle(m);
                      return (
                        <div key={i} className={`flex ${style.align}`}><div className={`max-w-[85%] p-2.5 rounded-2xl ${style.bg}`}>{m.attachment_url ? <a href={m.attachment_url} target="_blank"><img src={m.attachment_url} alt="" className="rounded-lg mb-1 max-w-full" /></a> : <p className="text-sm">{m.message}</p>}<p className="text-[10px] mt-1 opacity-60">{new Date(m.created_at).toLocaleTimeString('fr-FR', {hour:'2-digit', minute:'2-digit'})}</p></div>
                      );
                    })}
                  </div>
