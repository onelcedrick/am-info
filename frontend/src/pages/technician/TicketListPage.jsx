...
export default function TicketListPage() {
  ...
  return (
    <div className="h-full flex flex-col">
      ...
      {/* Header + Filtres - toujours visible */}
      <div className="flex-shrink-0 space-y-3 mb-4">
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-3 flex gap-2 flex-wrap">
          ...
          <select value={filterStatus} onChange={e => setFilterStatus(e.target.value)} className="px-3 py-2 border rounded-lg text-sm">
            <option value="all">Tous</option><option value="open">Ouvert</option><option value="assigned">Assigné</option><option value="in_progress">En cours</option><option value="resolved">Résolu</option><option value="closed">Fermé</option>
          </select>
            ...
            {tickets.length === 0 ? <div className="text-center text-gray-400 py-8">Aucun ticket</div> :
              tickets.map(t => (
                <div key={t.id} onClick={() => selectTicket(t)}
                  className={`bg-white rounded-xl shadow-sm border p-4 cursor-pointer hover:shadow-md transition ${selectedTicket?.id === t.id ? 'ring-2 ring-teal-500 border-teal-500' : 'border-gray-100'}`}>
                  <div className="flex justify-between items-start mb-2">
                    <div className="flex items-center gap-2 flex-1 min-w-0">
                      <span className="text-xs flex-shrink-0">{priorityIcons[t.priority] || '🔵'}</span>
                      <h3 className="font-bold text-sm truncate">{t.subject}</h3>
                    </div>
                    <span className={`ml-2 px-2 py-0.5 rounded-full text-xs font-semibold flex-shrink-0 ${statusColors[t.status]}`}>{statusLabels[t.status]}</span>
                  </div>
                  <p className="text-xs text-gray-500 mb-2 line-clamp-2">{t.description}</p>
                  <div className="flex justify-between items-center">
                    <span className="text-xs text-gray-400">{priorityLabels[t.priority]} · {slaTargets[t.priority]}</span>
                    {t.status === 'open' ? (
                      ...
                      <select value={t.status} onClick={e => e.stopPropagation()} onChange={e => { e.stopPropagation(); changeStatus(t.id, e.target.value); }}
                        className={`px-2 py-0.5 rounded-full text-xs font-semibold cursor-pointer ${statusColors[t.status]}`}>
                        <option value="assigned">Assigné</option><option value="in_progress">En cours</option><option value="resolved">Résolu</option><option value="closed">Fermé</option>
                      </select>
                    )}
                  </div>
                </div>
              ...
              <div className="bg-white rounded-xl shadow-sm border border-gray-100 flex flex-col h-full">
                <div className="p-4 border-b flex-shrink-0">
                  <div className="flex justify-between items-start mb-2">
                    ...
                    {typing && <span className="text-xs text-gray-400 animate-pulse">Client écrit...</span>}
                  </div>
                  {selectedTicket.sla && (
                    <div className={`flex items-center gap-2 p-2 rounded-lg border text-xs ${getSlaBgColor(selectedTicket.sla.sla_color)}`}>
                      <span>{priorityIcons[selectedTicket.priority]}</span>
                      <div className="flex-1">
                        <div className="flex justify-between">
                          <span className="font-semibold">{selectedTicket.sla.priority_label} · {selectedTicket.sla.sla_message}</span>
                          <span className="font-bold">{selectedTicket.sla.elapsed_minutes} min</span>
                        </div>
                        <div className="w-full h-1 bg-white/50 rounded-full mt-1">
                          <div className={`h-full rounded-full ${getSlaColor(selectedTicket.sla.sla_color)}`}
                ...
                <div ref={messagesContainerRef} onScroll={handleScroll} className="flex-1 overflow-y-auto p-3 space-y-3 bg-gray-50">
                  {!autoScroll && (
                    <div className="sticky top-0 text-center z-10">
                      <button onClick={scrollToBottom} className="bg-teal-600 text-white px-3 py-1 rounded-full text-xs shadow-lg">Nouveaux messages</button>
                    </div>
                  )}
                    ...
                    return (
                      <div key={i} className={`flex ${style.align} group relative`}>
                        <div className={`max-w-[85%] p-2.5 rounded-2xl ${style.bg} relative`}>
                          ...
                          {isEditing ? (
                            <div className="flex gap-1"><input value={editText} onChange={e => setEditText(e.target.value)} className="flex-1 px-2 py-1 border rounded text-xs" autoFocus onKeyPress={e => e.key === 'Enter' && saveEdit(m.id)} /><button onClick={() => saveEdit(m.id)} className="text-green-500 font-semibold text-xs">OK</button></div>
                          ) : (
                            <>{m.attachment_url ? <a href={m.attachment_url} target="_blank"><img src={m.attachment_url} alt="" className="rounded-lg mb-1 max-w-full" /></a> : <p className="text-sm">{m.message}</p>}<p className="text-[10px] mt-1 opacity-60">{new Date(m.created_at).toLocaleTimeString('fr-FR', {hour:'2-digit', minute:'2-digit'})}</p></>
                          )}
                          {!isBot && !isEditing && (
                            <div className="hidden group-hover:flex absolute -top-2 right-2 gap-1">
                              {isMine && <button onClick={(e) => { e.stopPropagation(); startEdit(m); }} className="bg-blue-500 text-white rounded-full w-5 h-5 flex items-center justify-center text-[10px]" title="Modifier">✎</button>}
                              <button onClick={(e) => { e.stopPropagation(); deleteMessage(m.id); }} className="bg-red-500 text-white rounded-full w-5 h-5 flex items-center justify-center text-[10px]" title="Supprimer">✕</button>
                            </div>
                          )}
                        </div>
                      </div>
                ...
                {preview && (
                  <div className="px-3 py-2 border-t bg-gray-50 flex items-center gap-2 flex-shrink-0">
                    <img src={preview} alt="" className="h-8 w-8 object-cover rounded" />
                    <button onClick={uploadPhoto} disabled={uploading} className="ml-auto bg-teal-600 text-white px-2 py-1 rounded text-xs">{uploading ? '...' : 'Envoyer'}</button>
                    <button onClick={() => { setPreview(null); fileInputRef.current.value = ''; }} className="text-red-400 text-xs">Annuler</button>
                  </div>
                )}

                <div className="p-3 border-t flex gap-2 flex-shrink-0">
                  <input type="file" accept="image/*" ref={fileInputRef} onChange={handleFileSelect} className="hidden" id="tech-file-upload" />
                  <label htmlFor="tech-file-upload" className="bg-gray-100 hover:bg-gray-200 px-2 py-2 rounded-full cursor-pointer text-sm">+</label>
                  <input placeholder="Réponse..." value={text} onChange={e => setText(e.target.value)} onKeyPress={e => e.key === 'Enter' && send()}
                ...
                tickets.map(t => (
                  <div key={t.id} onClick={() => selectTicket(t)}
                    ...
                    <div className="flex justify-between items-start mb-1">
                      ...
                      <span className={`ml-1 px-1.5 py-0.5 rounded-full text-[10px] font-semibold ${statusColors[t.status]}`}>{statusLabels[t.status]}</span>
                    </div>
                    <p className="text-xs text-gray-500 mb-1 line-clamp-1">{t.description}</p>
                    <div className="flex justify-between items-center">
                      <span className="text-[10px] text-gray-400">{slaTargets[t.priority]}</span>
                      {t.status === 'open' ? (
                        <button onClick={e => { e.stopPropagation(); assignToMe(t.id); }} className="bg-teal-600 text-white px-2 py-0.5 rounded-full text-[10px]">Prendre</button>
                      ) : (
                        <span className="text-[10px] text-gray-500">{statusLabels[t.status]}</span>
                      )}
                    </div>
                  </div>
                ))}
              ...
              {selectedTicket && (
                <>
                  <div className="flex items-center gap-2 p-3 border-b bg-white flex-shrink-0">
                    <button onClick={backToList} className="text-teal-600 text-sm font-semibold">← Retour</button>
                      ...
                      return (
                        <div key={i} className={`flex ${style.align}`}>
                          <div className={`max-w-[85%] p-2.5 rounded-2xl ${style.bg}`}>
                            {style.label && <p className={`text-[10px] font-semibold mb-1 ${isBot ? 'text-blue-600' : 'text-gray-500'}`}>{style.label}</p>}
                            {m.attachment_url ? <a href={m.attachment_url} target="_blank"><img src={m.attachment_url} alt="" className="rounded-lg mb-1 max-w-full" /></a> : <p className="text-sm">{m.message}</p>}
                  ...
                  <div className="p-2 border-t flex gap-2 bg-white flex-shrink-0">
                    <input placeholder="Réponse..." value={text} onChange={e => setText(e.target.value)}
                      className="flex-1 px-3 py-2 border rounded-full text-sm" />
