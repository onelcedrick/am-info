  ...
  return (
    <div className="space-y-8 md:space-y-12 px-2 md:px-0">
      ...
      <div className="relative overflow-hidden bg-gradient-to-br from-blue-600 via-blue-700 to-indigo-800 text-white rounded-2xl md:rounded-3xl p-6 md:p-12 lg:p-16">
        <div className="relative z-10">
          ...
          <div className="flex gap-3 md:gap-4 flex-wrap">
            ...
            {!isAuthenticated && <Link to="/register" className="border-2 border-white text-white px-5 md:px-8 py-2.5 md:py-3 rounded-full text-sm md:text-base font-semibold hover:bg-white/10 transition">Creer un compte</Link>}
          </div>
      ...
      {newArrivals.length > 0 && (
        <div>
          <div className="flex justify-between items-center mb-4 md:mb-6">
            <div><h2 className="text-2xl md:text-3xl font-bold">Nouveaux arrives</h2><p className="text-gray-500 text-sm mt-1">Decouvrez nos derniers produits</p></div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-6">
            {newArrivals.map(p => (
              <div key={p.id} className="bg-white rounded-2xl shadow hover:shadow-lg transition overflow-hidden group relative">
                ...
                <Link to={`/products/${p.id}`}><div className="h-32 md:h-48 bg-gray-100 flex items-center justify-center overflow-hidden">{p.image_url?.startsWith('http') ? <img src={p.image_url} alt="" className="w-full h-full object-cover group-hover:scale-105 transition" /> : <svg className="w-10 h-10 md:w-16 md:h-16 text-gray-300" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1} d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z" /></svg>}</div></Link>
                <div className="p-3 md:p-4">
                  ...
                  {isAuthenticated && <button onClick={() => addToCart(p.id, p.name)} className="w-full mt-2 bg-blue-600 text-white py-1.5 rounded-lg text-xs hover:bg-blue-700">Ajouter</button>}
                </div>
              </div>
            ))}
          </div>
      ...
      {popularProducts.length > 0 && (
        <div>
          <div className="flex justify-between items-center mb-4 md:mb-6">
            <div><h2 className="text-2xl md:text-3xl font-bold">Produits populaires</h2><p className="text-gray-500 text-sm mt-1">Les plus demandes</p></div>
            <Link to="/products" className="text-blue-600 hover:underline font-semibold text-sm">Voir tout</Link>
          </div>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-6">
      ...
      <div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-6">
          {['Vente Materiel', 'Depannage', 'En Boutique', 'Garantie'].map(title => (
            <div key={title} className="bg-white rounded-2xl shadow p-4 md:p-6 text-center hover:shadow-lg transition">
              <div className="w-12 h-12 md:w-16 md:h-16 bg-blue-100 rounded-2xl flex items-center justify-center mx-auto mb-3"><svg className="w-6 h-6 md:w-8 md:h-8 text-blue-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 4v10M4 7v10l8 4" /></svg></div>
              <h3 className="font-bold text-sm md:text-lg mb-1">{title}</h3>
              <p className="text-gray-500 text-xs">Service de qualite</p>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="bg-white rounded-2xl md:rounded-3xl shadow p-6 md:p-12">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 md:gap-8">
          {['Creez un compte', 'Commandez ou demandez de l\'aide', 'Paiement en boutique'].map((t, i) => (
            <div key={t} className="text-center">
              <div className="w-10 h-10 md:w-12 md:h-12 bg-blue-600 text-white rounded-full flex items-center justify-center text-lg font-bold mx-auto mb-3">{i+1}</div>
              <h3 className="font-bold text-base md:text-lg mb-1">{t}</h3>
              <p className="text-gray-500 text-sm">Simple et rapide</p>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="bg-gradient-to-r from-gray-800 to-gray-900 text-white rounded-2xl md:rounded-3xl p-6 md:p-12 text-center">
        ...
        <div className="flex flex-col md:flex-row gap-3 md:gap-4 justify-center">
          <Link to={isAuthenticated ? '/client/tickets' : '/login'} className="bg-blue-600 text-white px-6 md:px-8 py-2.5 md:py-3 rounded-full text-sm font-semibold hover:bg-blue-700">Demander de l'aide</Link>
          <Link to="/map" className="border border-gray-500 text-white px-6 md:px-8 py-2.5 md:py-3 rounded-full text-sm font-semibold hover:bg-white/10">Notre boutique</Link>
        </div>
      </div>
    </div>
  );
